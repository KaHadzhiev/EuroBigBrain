#!/usr/bin/env python3
"""Walk-forward on the 8-strategy EUR+GBP cross-asset portfolio (production champion)."""
import sys, json
...
def run_combined_window(start, end):
    logs = []
    for w in STRATEGIES:
    ...
    if not logs:
        return None
    return combine_strategies(logs, deposit=1000.0, max_concurrent_positions=4, overlap_penalty=0.5)


def main():
    print(f"=== 8-strategy walk-forward (9 folds, 6mo OOS each) ===")
    print(f"\n{'fold':<22}{'trades':>8}{'PF':>8}{'WR':>6}{'DD%':>7}{'Sharpe':>8}{'PnL$':>10}")
    print('-' * 70)
    rows = []
    for s, e in FOLDS:
        m = run_combined_window(s, e)
        if m is None:
            print(f"  {s} no trades"); continue
        rows.append({"start": s, "end": e, "n_trades": m.n_trades,
                     "pf": m.pf, "wr": m.win_rate, "max_dd_pct": m.max_dd_pct,
    ...
    op = RESULTS / "eur_gbp_8strat_walkforward.json"
