#!/usr/bin/env python3
"""10-strategy portfolio: 8-strat + 2 NEW GBP M30 (breakout_FullDay + asian_range_London).

All 10 are individually null-validated (5x hour-shuffle gate).
"""
import sys, json
...
def main():
    tag = "eur_gbp_10strat_v1"
    logs = []
    print("=== 10-strategy cross-asset portfolio ===")
    for w in STRATEGIES:
        log = run_one(w)
        ...
        wr = (log["pnl"] > 0).mean()
        net = log["pnl"].sum()
        print(f"  {w['name']:35s}  trades={len(log):4d}  PF={pf:.3f}  net=${net:+.0f}  null={w['null_edge']:.1f}x")
        log.to_csv(RESULTS / f"{tag}__{w['name']}__trades.csv", index=False)
        logs.append(log)

    print("\n[combine 10-strat]")
    m = combine_strategies(logs, deposit=1000.0, max_concurrent_positions=5, overlap_penalty=0.5)
